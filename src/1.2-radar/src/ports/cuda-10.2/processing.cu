...
__device__ cuComplex cuCpolarf(float r, float theta)
{
	return make_cuComplex(r * cos(theta), r * sin(theta));
}

__device__ float cuCargf(cuComplex z)
{
...
__global__ void SAR_DCE(float *data, radar_params_t *params, float const_k)
{
    ...
    cuComplex *c_data = (cuComplex*) data;
    extern __shared__ cuComplex tmp[];

    tmp[i] = cuCmulf(cuConjf(c_data[i*off+j]), c_data[(i+1)*off+j]);
    for(int k = 1; k < cell_x_thread; k++){
        int l = i + BLOCK_SIZE * k;
        if (l < params->apatch-1) tmp[i] = cuCaddf(tmp[i],cuCmulf(cuConjf(c_data[l*off+j]), c_data[(l+1)*off+j]));
    }
    __syncthreads();

    for (int s = blockDim.x/2; s > 0; s >>= 1){
        if (i < s) tmp[i] = cuCaddf(tmp[i], tmp[i+s]);
        __syncthreads();
    }
    if (i == 0){
        float val = cuCargf(tmp[0]);
        val = val*const_k;
        __threadfence();
        atomicAdd(&fDc, val);
    }
...
__global__ void SAR_ref_product(float *data, float *ref, uint32_t w, uint32_t h)
{
    ...
    cuComplex *c_data = (cuComplex*) data;
    cuComplex* c_ref = (cuComplex*) ref;
    c_data[i*w+j] = cuCmulf(c_data[i*w+j], cuConjf(c_ref[j]));
}
...
__global__ void SAR_rcmc(float *data, uint32_t *offsets, uint32_t width, uint32_t height)
{
    ...
    cuComplex *c_data = &((cuComplex*) data)[(blockIdx.y*height) * width];
    for(uint32_t i = 0; i < height; i++)
    {
        uint32_t ind = i * width + j;
        if(offsets[ind]<height) c_data[ind] = c_data[j+offsets[ind]*width];
        else c_data[ind] = make_cuComplex(0,0);
    }
}
...
{
    cuComplex *c_data = (cuComplex*) radar_data;
    ...
    for(int iy = 0; iy < isy; iy++)
        for(int jx = 0; jx < isx; jx++)
            fimg += cuCabsf(c_data[initIdx+iy*range_w+jx]);
...
__global__ void bin_reverse(float *data, unsigned int size, unsigned int group)
{
    ...
    cuComplex *c_data = (cuComplex*) &data[i*size*2];
    unsigned int j = 0;
