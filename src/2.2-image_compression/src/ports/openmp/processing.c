...
void ccsds_wavelet_transform_1D(const int* A, int* B, const int size, const int step = 1){
	...
	// high part
	B[size*step] = A[1*step] - floor( ((9.0/16.0) * (A[0] + A[2*step])) - ((1.0/16.0) * (A[2*step] + A[4*step])) + (1.0/2.0)); 
	B[(2*size-2)*step] = A[(2*size - 3)*step] - floor( ((9.0/16.0) * (A[(2*size -4)*step] + A[(2*size -2)*step])) - ((1.0/16.0) * (A[(2*size - 6)*step] + A[(2*size - 2)*step])) + (1.0/2.0));
	B[(2*size-1)*step] = A[(2*size - 1)*step] - floor( ((9.0/8.0) * (A[(2*size -2)*step])) -  ((1.0/8.0) * (A[(2*size - 4)*step])) + (1.0/2.0));

	#pragma omp parallel for
	for (unsigned int i = 1; i < size-2; ++i){
		//store
		B[(i+size)*step] = A[(2*i+1)*step] - floor( ((9.0/16.0) * (A[(2*i)*step] + A[(2*i+2)*step])) - ((1.0/16.0) * (A[(2*i - 2)*step] + A[(2*i + 4)*step])) + (1.0/2.0));
	}

	// low_part
	B[0] = A[0] - floor(- (B[size*step]/2.0) + (1.0/2.0));
	#pragma omp parallel for
	for (unsigned int i = 1; i < size; ++i){	
		B[i] = A[(2*i)*step] - floor( - (( B[(i + size -1)*step] +  B[(i + size)*step])/ 4.0) + (1.0/2.0) );	
	}
...
void ccsds_wavelet_transform_2D (int** A, const int w_size, const int h_size){
	...
	for (unsigned int i = 0; i < h_size; ++i){   
        ccsds_wavelet_transform_1D(A[i], auxiliar + i * w_size, w_size/2, 1);
    }
    ...
    for (unsigned int i = 0; i < w_size; ++i){
        ...
        ccsds_wavelet_transform_1D(auxiliar_h_input, auxiliar_h_output, h_size/2,1);
		for(unsigned int x = 0; x < h_size; ++x ){
