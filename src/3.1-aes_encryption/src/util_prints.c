    ...
    )
{
    if(!print_info_data->csv_mode && !print_info_data->database_mode){
        ...
        printf("Plaintext length (bytes) = %d \n", print_info_data->d_size);
        printf("Key size (bits)          = %d \n", print_info_data->k_size);
        printf("Nb                       = %d \n", print_info_data->Nb);
        printf("Nk                       = %d \n", print_info_data->Nk);
        printf("Nr                       = %d \n", print_info_data->Nr);
    ...
    )
{
    ...
    // calculate the throughput in MBytes/s
    float throughput = (print_info_data->d_size) / (total_time * 1000);
