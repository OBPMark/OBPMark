/**
 * \file util_prints.h 
 * \brief Benchmark #2.1 common print utils.
    ...
    )
{
    if(!print_info_data->csv_mode && !print_info_data->database_mode){
        ...
        printf("Number of samples       = %d \n", print_info_data->num_samples);
        printf("Bit size                = %d \n", print_info_data->num_bits);
        printf("Sample interval         = %d \n", print_info_data->sample_interval);
        printf("Compression block size  = %d \n", print_info_data->block_size);
        printf("Preprocessor active     = ");
        printf(BOOL_PRINT(print_info_data->preprocessor_active));
        printf("\n");
    ...
    )
{
    ...
    float throughput = (print_info_data->num_samples) / (total_time * 1000);
    ...
    if (print_info_data->csv_mode)
	{
        if (print_info_data->extended_csv_mode)
        {
            ...
            print_info_data->sample_interval, 
            print_info_data->block_size,
            print_info_data->preprocessor_active,
            host_to_device_time, 
            execution_time, 
            device_to_host_time, 
