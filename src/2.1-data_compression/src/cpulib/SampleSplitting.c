...
struct FCompressedData SampleSplitting(unsigned int* Samples, unsigned int k)
{
    ...
    if(k >= n_bits - 2)
    {
        ...
        //printf("Sample Splitting (k >= n_bits - 3): The data stream won't be compressed.\n");
    ...
    else if(k == 0)
    {
        ...
        //printf("Sample Splitting (k == 0): Performing Fundamental Sequence.\n");
    ...
    if(CompressedSize > J_BlockSize * 32)
    {
        ...
        //printf("Sample Splitting k = %d (Compressed size: %d bits > %d bits): Error.\n", k, CompressedSize, J_BlockSize * 32);
        CompressedData.CompressionIdentifier = SAMPLE_SPLITTING_ID + k;
        return CompressedData;
    }
    ...
    //printf("Sample Splitting k = %d (Size: %d bits): OK.\n", k, CompressedSize);
